...
ASSOC=/cellar/users/mpagadal/projects/germline-immune3/data/plink-associations/associations

cd $ASSOC

/cellar/users/mpagadal/Programs/plink2 --bfile $GENO --pheno $PHENO --pheno-name $pheno --covar $COVAR --covar-name C1-C10,age --covar-variance-standardize --glm firth-fallback sex cols=+err --adjust --out $ASSOC/$pheno
...
rm $ASSOC/$pheno.$pheno.glm.linear.adjusted
# rm $ASSOC/$pheno.log
